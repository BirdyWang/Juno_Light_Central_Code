 ...
 *  rate        Minimum sampling rate, or zero to disable LP
 *              accel mode.
 *  0 if successful.
 */
...
int mpu_set_accel_bias_6500_reg(const long *accel_bias)
{
    ...
    data[1] = (accel_reg_bias[0] >> 8) & 0xff;
...
int mpu_reset_fifo(void)
{
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        if (st.chip_cfg.bypass_mode || !(st.chip_cfg.sensors & INV_XYZ_COMPASS))
            ...
            data[1] = BIT_FIFO_EN | BIT_AUX_IF_EN;
        data[0] = st.reg->user_ctrl + MPU6500_WRITE;
        nrf_delay_ms(50);
        if (st.chip_cfg.int_enable)
            data[1] = BIT_DATA_RDY_EN;
        else
            data[1] = 0;
        data[0] = st.reg->int_enable + MPU6500_WRITE;
        IMU_SPI_Transmit(data, 2);
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    switch (fsr) {
    case 250:
        data[1] = INV_FSR_250DPS << 3;
        break;
    case 500:
        data[1] = INV_FSR_500DPS << 3;
        break;
    case 1000:
        data[1] = INV_FSR_1000DPS << 3;
        break;
    case 2000:
        data[1] = INV_FSR_2000DPS << 3;
        break;
    default:
        return -1;
    }
...
int mpu_set_lpf(unsigned short lpf)
{
    ...
    if (lpf >= 188)
        data[1] = INV_FILTER_188HZ;
    else if (lpf >= 98)
        data[1] = INV_FILTER_98HZ;
    else if (lpf >= 42)
        data[1] = INV_FILTER_42HZ;
    else if (lpf >= 20)
        data[1] = INV_FILTER_20HZ;
    else if (lpf >= 10)
        data[1] = INV_FILTER_10HZ;
    else
        data[1] = INV_FILTER_5HZ;
...
int mpu_set_int_latched(uint8_t enable)
{
    ...
    if (enable)
        tmp[1] = BIT_LATCH_EN | BIT_ANY_RD_CLR;
    else
        tmp[1] = 0;
        ...
        uint8_t *data)
{
    ...
    tmp[1] = (uint8_t)(mem_addr >> 8);
    tmp[2] = (uint8_t)(mem_addr & 0xFF);
...
#define LOAD_CHUNK      16
    ...
    for (ii = 0; ii < length; ii += this_write) {
        ...
        if (mpu_read_mem(ii, this_write, cur))
            return -1;
        if (memcmp((uint8_t*)&firmware[ii], cur, this_write))
            return -2;
    }
    ...
    uint8_t lpa_freq)
{
    ...
    if (lpa_freq) {
        ...
        if (lpa_freq == 1)
            ...
            data[1] = INV_LPA_160HZ;
        else if (lpa_freq <= 320)
            data[1] = INV_LPA_320HZ;
        else
